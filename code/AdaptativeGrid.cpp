...
void AdaptativeGrid::init(const std::vector<unsigned short>& indices, const std::vector<glm::vec3>& vertices, VoxelizationMethod method) {
...
void AdaptativeGrid::generateVoxels() {
...
VoxelData AdaptativeGrid::getVoxel(int x, int y, int z) {
...
int AdaptativeGrid::getColumnIndex(const VoxelData& voxel, int projectionAxis) const {
...
int AdaptativeGrid::getVoxelIndex(int x, int y, int z) const {
    return x * gridResolutionY * gridResolutionZ + y * gridResolutionZ + z;
}


bool AdaptativeGrid::intersectRayTriangle(const glm::vec3& rayOrigin, const glm::vec3& rayDir, 
...
void AdaptativeGrid::processRaycastingForAxis(const std::vector<unsigned short>& indices,
...
void AdaptativeGrid::voxelizeMeshSurface(const std::vector<unsigned short>& indices, const std::vector<glm::vec3>& vertices) {
    if (indices.size() % 3 != 0) {
        std::cerr << "Error: The index data is not valid. Must be a multiple of 3 (triangles)." << std::endl;
        return;
    }
...
void AdaptativeGrid::optimizedVoxelizeMesh(const std::vector<unsigned short>& indices, 
                                        const std::vector<glm::vec3>& vertices) {
   if (indices.size() % 3 != 0) {
        std::cerr << "Error: The index data is not valid. Must be a multiple of 3 (triangles)." << std::endl;
        return;
